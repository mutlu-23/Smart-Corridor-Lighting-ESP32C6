// =========================================================================
// Proje: Akıllı Koridor LED Aydınlatma Sistemi (Tek Dosya Sürümü)
// Hedef: ESP32-C6
// Yazar: Mutlu PEKER 
// Sürüm: 1.5.6 
// =========================================================================
...
#include <Update.h>
#include <type_traits>
#include <cstring> // strcmp, strlen için eklendi
// #include <esp_task_wdt.h> // WDT Şimdilik Devre Dışı

// ==================== AYARLAR ve SABİTLER ====================

// Donanım Pinleri (Sizin belirttiğiniz son çalışan değerler)
#define PIN_LED_DATA         4
#define PIN_RADAR_AMP        25 // Analog pin (ADC1_CH8 ESP32-C6'da yok, uygun bir GPIO seçilmeli. Varsayılan olarak 25 kullanılıyor, kontrol edin!)
...
#define LOG_BUFFER_SIZE    25 // Biraz daha artırıldı
#define APP_VERSION        "1.5.6" // Sürüm güncellendi
...
#define RADAR_AMP_SMOOTHING       0.6f // Daha yavaş yumuşatma (0.1 -> 0.9 arası, küçük değer daha yavaş)
#define POSITION_SMOOTHING        0.3f // Daha hızlı pozisyon takibi (0.1 -> 0.9 arası, büyük değer daha hızlı)

// ==================== VERİ YAPILARI ====================

// Sistem Konfigürasyonu (v2.0.0'dan eklemelerle)
struct SystemConfig {
  ...
  uint16_t motionThresholdAmplitude = 100; int16_t ledSegmentLength = 30; // Use int16_t for constrain compatibility if needed
  ...
  uint8_t currentEffect = EFFECT_TRAIL; uint8_t transitionSpeed = 10; // v2.0.0'dan eklendi
};

// Sistem Durumu (v2.0.0'dan eklemelerle)
struct SystemState {
  ...
  // v2.0.0'dan gelen ek durumlar (varsa eklenebilir, şimdilik gerek yok gibi)
  // uint8_t currentMode = MODE_NORMAL; // Bu isEvening/isSleepTime ile yönetiliyor
};

// ==================== YARDIMCI FONKSİYONLAR (Ön Tanımlar) ====================
void addLog(String message);
float mapFloat(float x, float in_min, float in_max, float out_min, float out_max);

// Arduino max/min makroları bazen std::max/min ile çakışabilir.
// İhtiyaç olursa diye explicit std:: kullanımı için veya kendi makromuzu tanımlamak için:
template<typename T> T safe_max(T a, T b) { return (a > b) ? a : b; }
template<typename T> T safe_min(T a, T b) { return (a < b) ? a : b; }


// ==================== SINIF TANIMLARI (EKLENDİ) ====================
...
private:
  ...
  const uint16_t _radarReadSamples = 5; // Gürültü azaltmak için okunacak örnek sayısı
...
public:
  bool begin() {
    if (!tsl.begin()) {
      ...
      tsl.enableAutoRange(true); // Otomatik kazanç ayarı
      tsl.setIntegrationTime(TSL2561_INTEGRATIONTIME_101MS); // Orta hız/hassasiyet
      _tslOk = true;
    }
    // Radar pini zaten INPUT olarak ayarlandı (setupPins içinde)
    // İlk yumuşatılmış değeri okuyalım
    _smoothedRadarValue = analogRead(PIN_RADAR_AMP);
    return _tslOk; // Veya her iki sensörün de durumuna bağlı bir değer dönebilir
  }
  ...
  void update() {
    ...
    if (now - _lastReadTime >= SENSOR_READ_INTERVAL_MS) {
      ...
      for (int i = 0; i < _radarReadSamples; i++) {
          ...
          delayMicroseconds(50); // ADC okumaları arasında kısa bekleme
      }
      _rawRadarValue = radarSum / _radarReadSamples;


      // Radar Yumuşatma (Exponential Moving Average)
      ...
      if (_tslOk) {
        ...
        tsl.getEvent(&event);
        if (event.light >= 0) { // Geçerli bir değer okunduysa
             ...
             _tslOk = false; // Sensör hataya geçti
             _currentLux = 0.0f; // Hata durumunda varsayılan değer
        }
      } else {
          // TSL çalışmıyorsa, belirli aralıklarla tekrar kontrol etmeyi deneyebiliriz
          // if (now % 60000 == 0) { // Dakikada bir tekrar dene
          //   if (tsl.begin()) { _tslOk = true; /* ... */ }
          // }
           _currentLux = 0.0f; // Sensör yoksa varsayılan
      }
    }
  }
  ...
  float calculateDistance(uint16_t amplitude, const SystemConfig& config) {
    // Önce gürültü tabanının altındaki değerleri filtrele
    if (amplitude <= config.radarNoiseFloor) {
        return -1.0f; // Gürültü seviyesinde veya altında, mesafe yok
    }

    // Kalibrasyon noktalarını kontrol et (mantıksızsa hesaplama yapma)
    // Mesafe arttıkça genlik artmalı!
    if (!(config.radarAmplitudeAt5m >= config.radarAmplitudeAt3m &&
      ...
      // Kalibrasyon noktaları mantıklı sırada değilse
      static uint32_t lastCalibErrorLog = 0;
      if (millis() - lastCalibErrorLog > 300000) { // 5 dakikada bir logla
          addLog("UYARI: Mesafe tahmini için radar kalibrasyonu geçersiz!");
          lastCalibErrorLog = millis();
      }
      return -1.0f; // Geçersiz kalibrasyon
    }

    // Mesafe haritalama (segment bazlı mapFloat)
    float distance = -1.0f;
    ...
    // Mesafe arttıkça genliğin ARTTIĞINI varsayıyoruz.

    if (amplitude <= amp1m) { // 0-1m arası (noise ile 1m arası)
         distance = mapFloat(amplitude, noise, amp1m, 0.0f, 1.0f);
    } else if (amplitude <= amp3m) { // 1-3m arası
         distance = mapFloat(amplitude, amp1m, amp3m, 1.0f, 3.0f);
    } else if (amplitude <= amp5m) { // 3-5m arası
         distance = mapFloat(amplitude, amp3m, amp5m, 3.0f, 5.0f);
    } else { // 5m üzeri (Maksimum ölçülen genlikten büyük)
        // Burada bir ekstrapolasyon yapılabilir veya sabit bir değer (örn. 5.0m) döndürülebilir.
        // Şimdilik 5m olarak kabul edelim.
        distance = 5.0f; // Veya CORRIDOR_LENGTH_M;
    }

    // Mesafeyi koridor uzunluğu ile sınırla
  ...
  uint16_t calibrateNoiseFloor() {
      ...
      for (int i = 0; i < numSamples; ++i) {
          ...
          delay(10); // Örnekler arası bekleme
      }
      uint16_t noiseFloor = sum / numSamples;
      Serial.println("Gürültü tabanı ölçüldü: " + String(noiseFloor));
      // Yumuşatılmış değeri de güncelleyebiliriz belki? Şimdilik gerek yok.
      // _smoothedRadarValue = noiseFloor;
      return noiseFloor;
  }

  uint16_t captureCurrentAmplitude() {
    // Kalibrasyon sırasında anlık, biraz daha stabilize edilmiş değeri almak iyi olabilir.
    // update() içindeki yumuşatılmış değeri kullanalım.
    // Veya burada da birkaç okuma yapıp ortalama alabiliriz.
    // Şimdilik smoothed değeri dönelim.
...
public:
    ...
    // Tüm LED'leri belirli bir ölçekte karartır
    ...
    // Basit: Belirli bir pozisyon etrafındaki segmenti yakar
    void simpleEffect(float position, uint16_t segmentLength, CHSV color) {
        if (!_leds || _numLeds <= 0) return;
        fadeToBlack(50); // Önceki izleri biraz söndür
        int centerLed = round(position);
        int startLed = safe_max(0, centerLed - (int)(segmentLength / 2)); // Cast to int for safety
        int endLed = safe_min(_numLeds - 1, centerLed + (int)(segmentLength / 2)); // Cast to int
    ...
    // İz Efekti: Yanan segmentin arkasında sönük bir iz bırakır
    void trailEffect(float position, uint16_t segmentLength, CHSV color) {
        if (!_leds || _numLeds <= 0) return;
        fadeToBlack(30); // Daha yavaş söndürerek iz bırak
        simpleEffect(position, segmentLength, color); // Ana segmenti yak
    }

    // Nabız Efekti: Segmentin parlaklığı artıp azalır
    void pulseEffect(float position, uint16_t segmentLength, CHSV baseColor) {
        ...
        if (now - _lastEffectUpdateTime > 20) { // Nabız hızı
            _lastEffectUpdateTime = now;
             // Basit artış/azalış
            if (_pulseDirection) {
                 _pulseBrightness = safe_min((uint8_t)250, (uint8_t)(_pulseBrightness + 5)); // Use safe_min/max
                 if (_pulseBrightness >= 250) _pulseDirection = false;
            } else {
                 _pulseBrightness = safe_max((uint8_t)10, (uint8_t)(_pulseBrightness - 5)); // Use safe_min/max
                 if (_pulseBrightness <= 10) _pulseDirection = true;
            }
             // _pulseBrightness = constrain(_pulseBrightness, 10, 250); // constrain is also fine
        }

        CHSV pulseColor = baseColor;
        pulseColor.v = _pulseBrightness; // Parlaklığı ayarla

        for (int i = startLed; i <= endLed; ++i) {
             if (i >= 0 && i < _numLeds) { // Bounds check
                 _leds[i] = pulseColor;
             }
        }
    }

    // Gökkuşağı Segmenti: Belirtilen segmenti gökkuşağı renklerinde yakar
    void rainbowEffect(float position, uint16_t segmentLength) {
        ...
        if (numToFill > 0 && startLed < _numLeds) {
            uint8_t initialHue = (millis() / 20) % 256; // Zamanla değişen başlangıç rengi
            // Ensure we don't write past the end of the array
            numToFill = safe_min(numToFill, _numLeds - startLed);
            fill_rainbow(&(_leds[startLed]), numToFill, initialHue, 7); // 7 hue adımı
        }
    }

    // Tiyatro Takibi: Segment içinde koşan ışıklar
    void theaterChaseEffect(float position, uint16_t segmentLength, CHSV color) {
        if (!_leds || _numLeds <= 0) return;
        fadeToBlack(60); // Önceki durumu temizle/söndür
        ...
        uint32_t now = millis();
         // Efekt hızını ayarlamak için _lastEffectUpdateTime kullanılır
        if (now - _lastEffectUpdateTime > 100) { // Koşma hızı (ms)
            _theaterChaseOffset = (_theaterChaseOffset + 1) % 3; // 3 adımda bir tekrar et
            _lastEffectUpdateTime = now;
        }
        ...
        for (int i = startLed; i <= endLed; i++) {
            if (i >= 0 && i < _numLeds) { // Bounds check
                if ((i % 3) == _theaterChaseOffset) {
                    _leds[i] = color; // Her 3 LED'den birini yak
                } else {
                    _leds[i] = CRGB::Black; // Diğerlerini söndür (fadeToBlack zaten yaptı ama garanti olsun)
                }
            }
        }
    }
...
// ==================== GÖMÜLÜ WEB VARLIKLARI (v2.0.0 Arayüzü) ====================
// (index_html, style_css, script_js - Değişiklik yok, önceki ile aynı)
...
// ---------- style.css (v2.0.0 Arayüzü için stiller) ----------
...
// ---------- script.js (v1.5.0 - Gelişmiş Arayüz Kontrolleri) ----------
...
// --- GÜNCELLENMİŞ uploadFirmware (v1.4.1 uyumlu) ---
function uploadFirmware() {
    ...
    // Button'ı form içinde daha spesifik seçelim
    ...
    // Sunucudaki ESPAsyncWebServer upload handler'ı 'update' ismini bekler
    formData.append('update', file, file.name); // Parametre adını 'update' yap

    const xhr = new XMLHttpRequest();
    xhr.open('POST', '/update', true); // Endpoint /update olmalı
...
// --- ANLIK KONTROL OLAY DİNLEYİCİLERİ ---
...
SystemConfig config;
SystemState state;
SensorManager sensorManager; // Sensör yönetimi nesnesi (Sınıf tanımından sonra)
LedControl ledControl;       // LED kontrol nesnesi (Sınıf tanımından sonra)
...
void setupFastLED();
void setupOTA();
void addLog(String message); // Zaten yukarıda tanımlandı
String getLogsJson();
void processRadar();
void updateSystemState();
void updateLeds();
void setAllLEDs(CRGB color); // Prototip eklendi
...
void setup() {
    ...
    // SensorManager.begin() çağrısı setupSensors içine taşındı
    setupSensors();

    // LedControl.begin() çağrısı setupFastLED içine taşındı
    setupFastLED();

    // setAllLEDs(CRGB::Black); // setupFastLED içinde yapıldı
    ...
    state.lastMotionDetectedTime = millis() - MOTION_TIMEOUT_MS - 1000; // Başlangıçta hareket yok say
}
...
void loop() {
    ...
    checkWifiConnection(); // WiFi bağlantısını kontrol et/yeniden bağlanmayı dene
    if (state.wifiConnected) {
        ArduinoOTA.handle(); // OTA güncellemelerini dinle
    }

    ws.cleanupClients(); // WebSocket bağlantılarını temizle

    sensorManager.update(); // Sensörleri güncelle (içinde zaman kontrolü var)
    processRadar();         // Radarı işle (sensör verilerini kullanır)

    updateSystemState();    // Genel sistem durumunu güncelle (ışık seviyesi, modlar vs.)

    updateLeds();           // LED'leri güncelle (sistem durumuna ve efektlere göre, içinde zaman kontrolü var)

    // Periyodik WebSocket durum güncellemesi
    ...
    delay(1); // Diğer görevlere zaman tanımak için küçük bir gecikme
}
...
void setupPins() {
    ...
    // ESP32-C6 için doğru ADC kanalını/pinini kullandığınızdan emin olun.
    // `analogRead()` fonksiyonu GPIO pin numarasını alır.
    // Gerekirse ADC yapılandırması eklenebilir (örn. `adcAttachPin()`, `analogSetAttenuation()`)
    // Varsayılan olarak bırakılmıştır.
...
bool setupWiFi() {
    ...
    if(WiFi.status() == WL_CONNECTED) {
        ...
        WiFi.disconnect(false); // Bağlantıyı kes ama ayarları silme
...
void setupAPMode() {
    ...
    if(result) {
        ...
        state.apMode = false; // Başlatılamadıysa AP modunda değiliz
    }
}
...
void setupLittleFS() {
    if (!LittleFS.begin(true)) { // true -> format if mount failed
        ...
        // Burada belki bir LED paterni ile hata belirtilebilir
...
bool loadConfiguration() {
    ...
    // Yüklenen değerleri doğrula (örn. sınırlar içinde mi?)
...
bool saveConfiguration() {
    ...
    DynamicJsonDocument doc(2048); // Kapasiteyi kontrol et

    // Değerleri JSON'a aktar
    doc["wifi_ssid"] = config.wifi_ssid;
    // Şifre boş değilse kaydet (güvenlik? Belki hiç kaydetmemek daha iyi?)
    if (strlen(config.wifi_password) > 0) {
         doc["wifi_password"] = config.wifi_password;
    } // Eğer şifre kısmı web arayüzünde sadece değiştirilmek istendiğinde giriliyorsa,
      // boş şifre kaydetmemek mantıklı olabilir. Ancak config struct'ında tutuluyorsa
      // kaydetmek gerekir. Bu haliyle struct'taki değeri kaydeder.
...
void setupSensors() {
    if(!sensorManager.begin()) {
        // Hata mesajı zaten SensorManager.begin() içinde loglanıyor
        state.tslSensorOk = false; // Durumu güncelle
    } else {
        state.tslSensorOk = sensorManager.isLightSensorWorking(); // Başlangıç durumunu al
...
void setupFastLED() {
    ...
    // Güç limitini ayarla (isteğe bağlı ama önerilir)
    int max_mA = 18000; // Dikkat: Bu değer güç kaynağınıza ve kablolarınıza uygun olmalı!
    FastLED.setMaxPowerInVoltsAndMilliamps(5, max_mA);
    FastLED.setBrightness(0); // Başlangıçta kapalı
    ledControl.begin(leds, NUM_LEDS); // LedControl sınıfını başlat
    addLog("FastLED/LedControl başlatildi: 5V, " + String(max_mA) + "mA limit");
    setAllLEDs(CRGB::Black); // Tüm LED'leri başlangıçta söndür
    FastLED.show(); // Değişikliği uygula
}
...
void setupOTA() {
    ...
    ArduinoOTA
        .onStart([]() {
            ...
            // İsteğe bağlı: LED'leri söndür veya özel bir işaret ver
            setAllLEDs(CRGB::Blue); // Güncelleme sırasında mavi yap
        })
        .onEnd([]() {
            addLog("\nOTA Başarıyla Tamamlandı!");
            setAllLEDs(CRGB::Green); // Başarılı olunca yeşil yap
            delay(1000);
        })
        .onProgress([](unsigned int progress, unsigned int total) {
            ...
            int percent = (progress / (total / 100));
             if (percent != lastPercent) {
                ...
                lastPercent = percent;
                // İsteğe bağlı: LED ile ilerleme göstergesi
                 int litLeds = map(percent, 0, 100, 0, NUM_LEDS);
                 fill_solid(leds, NUM_LEDS, CRGB::Black);
                 // Ensure litLeds is within bounds
        ...
        .onError([](ota_error_t error) {
            ...
            setAllLEDs(CRGB::Red); // Hata durumunda kırmızı yap
            delay(2000);
        });
...
void setupWebServer() {
    ...
    // Firmware Güncelleme (HTTP Upload)
    ...
    server.on("/restore_config", HTTP_POST, [](AsyncWebServerRequest *r){
        // Bu sadece upload bittiğinde çağrılır, asıl iş handleRestoreUpload'da yapılır
        r->send(200, "text/plain", "Ayar dosyası yüklendi, işleniyor...");
        // Geri yükleme işleminin sonucunu burada bildirmek zor,
        // çünkü işlem handleRestoreUpload'un final kısmında tamamlanır.
        // Belki WS üzerinden bildirim yapılabilir.
    }, handleRestoreUpload);
...
void setupWebSockets() {
    ...
    server.addHandler(&ws); // WebSocket handler'ını web sunucusuna ekle
    addLog("WebSocket ayarlandı.");
}

// ==================== LOGLAMA FONKSİYONLARI ====================

// Yeni log mesajı ekler
void addLog(String message) {
    // Zaman damgası ekle
    ...
    Serial.println(logEntry); // Seriyele de yazdır

    // Yeni log mesajını bağlı tüm WS istemcilerine gönder
    if (ws.count() > 0) {
        DynamicJsonDocument doc(message.length() + 100); // JSON overhead için pay bırak
        doc["type"] = "log_update";
...
// Tüm logları JSON formatında döndürür
String getLogsJson() {
    DynamicJsonDocument doc(LOG_BUFFER_SIZE * 150 + 100); // Buffer boyutuna göre ayarla
    doc["type"] = "logs";
    JsonArray logsArray = doc.createNestedArray("logs");

    // Logları doğru sırada (eskiden yeniye) ekle
    int startIndex = state.logBufferIndex; // En son eklenenin bir sonrası (boş veya en eski)
    for (int i = 0; i < LOG_BUFFER_SIZE; ++i) {
        int currentIndex = (startIndex + i) % LOG_BUFFER_SIZE;
        if (state.logBuffer[currentIndex].length() > 0) { // Boş olmayanları ekle
...
void processRadar() {
    ...
    // Hareket algılama (yumuşatılmış değer ve eşik kullanılarak)
    if (state.smoothedRadarAmplitude > motionThresholdValue) {
        // Eşiğin üzerindeyse hareket var sayılır
        if (!state.motionDetected) {
         ...
         state.lastMotionDetectedTime = millis(); // Hareket görüldüğü sürece zamanı güncelle
    } else {
        // Eşiğin altındaysa ve belirli bir süre geçtiyse hareketi durdur
        if (state.motionDetected && (millis() - state.lastMotionDetectedTime > 500)) { // 500ms bekleme süresi
            addLog("Hareket durdu (Timeout)");
            state.motionDetected = false;
        }
        // Eğer eşiğin altına yeni düştüyse, timeout süresi başlayacak
    }


    // Hareket varsa mesafe tahmini ve LED pozisyonu hesapla
    if (state.motionDetected || (millis() - state.lastMotionDetectedTime < MOTION_TIMEOUT_MS)) { // Timeout süresince de pozisyonu tut
        state.estimatedDistanceMeters = sensorManager.calculateDistance(state.smoothedRadarAmplitude, config);

        // Kalibrasyon geçerliliğini kontrol et (SensorManager'dan gelen bilgiye göre değil, config'e göre)
        // Mesafe arttıkça genlik artmalı! (ESP-Radar datasheet'ine göre)
        state.calibrationInvalid = !(config.radarAmplitudeAt5m >= config.radarAmplitudeAt3m &&
                                     config.radarAmplitudeAt3m >= config.radarAmplitudeAt1m &&
                                     config.radarAmplitudeAt1m > config.radarNoiseFloor); // Eşitlik kabul edilebilir ama > daha iyi

        if (state.calibrationInvalid) {
             // Kalibrasyon hatasını logla (çok sık olmasın)
            static uint32_t lastCalibErrorLog = 0;
            if(millis() - lastCalibErrorLog > 300000){ // 5 dakikada bir
                addLog("HATA: Radar kalibrasyon değerleri geçersiz/hatalı sıralı!");
                lastCalibErrorLog = millis();
            }
            state.estimatedDistanceMeters = -1.0f; // Geçersiz kalibrasyonda mesafe hesaplama
            state.targetPositionLED = -1.0f; // Hedef pozisyonu sıfırla
            // state.currentPositionLED = -1.0f; // Mevcut pozisyonu da sıfırlayabiliriz veya son bilinen yerde kalabilir
        } else {
            // Geçerli kalibrasyon ve mesafe
            if (state.estimatedDistanceMeters >= 0.0f) {
                // Mesafeyi LED pozisyonuna çevir
                ...
                // LED pozisyonunu yumuşat (hedefe doğru hareket)
                if (state.currentPositionLED < 0.0f || !previousMotionState) {
                    // İlk hareket algılandığında veya uzun süre sonra tekrar algılandığında direkt hedefe atla
                 ...
                 // Mesafe hesaplanamadı (gürültü altında veya kalibrasyon dışı) ama hareket var
                 state.targetPositionLED = -1.0f;
                 // state.currentPositionLED'i ne yapmalı? Belki son bilinen pozisyonda tutabiliriz.
            }
        }
    } else {
        // Hareket yok (ve timeout süresi doldu)
        state.estimatedDistanceMeters = -1.0;
        state.targetPositionLED = -1.0;
        // state.currentPositionLED = -1.0; // Hareket bitince pozisyonu sıfırla
    }
}
...
void updateSystemState() {
    ...
    // Işık seviyesine göre modları belirle
    if (!state.tslSensorOk) {
        // Sensör çalışmıyorsa, her zaman akşam modunda varsayalım
        state.isEvening = true;
        state.isSleepTime = false; // Uyku modunu aktif etme
         // Sensör hatasını logla (çok sık olmasın)
         static uint32_t lastTslErrorLog = 0;
         if (millis() - lastTslErrorLog > 300000) { // 5 dakikada bir
             addLog("UYARI: TSL2561 ışık sensörü okunamıyor, Akşam modu varsayılıyor.");
             lastTslErrorLog = millis();
         }
    } else {
        // Sensör çalışıyor, normal kontrol
    ...
    if (shouldBeOn && !state.lightsOn) {
        ...
        state.lightsOn = true;
        // Parlaklığı sıfırdan başlatıp fade-in yapacağız (updateLeds içinde)
        // state.currentBrightness = 0; // updateLeds içinde yönetiliyor
    }
    ...
    // Kalibrasyon durumunu tekrar kontrol et (processRadar içinde de ediliyor ama burada da olabilir)
     state.calibrationInvalid = !(config.radarAmplitudeAt5m >= config.radarAmplitudeAt3m &&
                                  config.radarAmplitudeAt3m >= config.radarAmplitudeAt1m &&
                                  config.radarAmplitudeAt1m > config.radarNoiseFloor);
}
...
// LED'leri mevcut duruma göre günceller
void updateLeds() {
    unsigned long currentMillis = millis();
    // LED güncelleme sıklığını kontrol et (~30 FPS)
    if (currentMillis - state.lastLedUpdateTime < LED_UPDATE_INTERVAL_MS) {
        return; // Henüz güncelleme zamanı gelmedi
    }
    state.lastLedUpdateTime = currentMillis;

    if (!state.lightsOn) {
        // Işıklar kapalıysa, yavaşça söndür
        ledControl.fadeToBlack(40); // Karartma hızı (scale değeri)
        // Parlaklık sıfırsa tekrar ayarlamaya gerek yok
        if (FastLED.getBrightness() > 0) {
            // Tamamen söndüğünden emin ol
            bool allOff = true;
        ...
        // Işıklar AÇIK
        // Hedef parlaklığı belirle (mod'a göre)
        uint8_t targetBrightness = state.isSleepTime ? config.nightBrightness : config.normalBrightness;
        uint8_t currentLedLibBrightness = FastLED.getBrightness(); // Kütüphanenin parlaklığı

        // Geçiş hızına göre parlaklığı ayarla (fade-in / fade-out)
        uint8_t step = map(config.transitionSpeed, 1, 20, 1, 15); // Geçiş hızını map ile adıma çevir
        ...
        // Parlaklığı uygula (değiştiyse)
        if (newBrightness != currentLedLibBrightness) {
            FastLED.setBrightness(newBrightness);
        }
        state.currentBrightness = newBrightness; // Sistem durumu parlaklığını güncelle

        // Hedef rengi belirle
        CHSV color = state.isSleepTime
                     ? CHSV(config.nightHue, config.nightSat, 255) // Gece modu rengi (parlaklık FastLED ile ayarlanıyor)
                     : CHSV(0, 0, 255); // Normal/Akşam modu (Beyaz, parlaklık FastLED ile)

        // Mevcut efekti al
        EffectType effect = (EffectType)config.currentEffect;
        float position = state.currentPositionLED; // Yumuşatılmış LED pozisyonu
        uint16_t segment = config.ledSegmentLength;

        // Efekti uygula
        if (position >= 0.0f) { // Geçerli bir pozisyon varsa
            switch(effect) {
                ...
                case EFFECT_RAINBOW: ledControl.rainbowEffect(position, segment); break; // Renk parametresi yok
                case EFFECT_THEATER: ledControl.theaterChaseEffect(position, segment, color); break;
                default:             ledControl.simpleEffect(position, segment, color); break; // Bilinmeyen efekt -> Basit
            }
        } else if (state.motionDetected) {
            // Pozisyon hesaplanamadı ama hareket var (örn. kalibrasyon hatası)
            // Belki tüm koridoru yakabiliriz? Veya segmenti başlangıçta?
            // Şimdilik tüm koridoru yakmayı deneyelim
             fill_solid(leds, NUM_LEDS, color);
             // Veya sadece başlangıç segmentini yak:
             // ledControl.simpleEffect(0.0f, segment, color);
        } else {
            // Pozisyon yok ve hareket yok (bu durum updateSystemState kontrolünden sonra olmamalı ama garanti)
            ledControl.fadeToBlack(30);
        }
    }

    // Hesaplanan LED renklerini ve parlaklığı fiziksel şeride gönder
    FastLED.show();
}

// Tüm LED'leri belirli bir renge ayarlar (genellikle başlangıç ve özel durumlar için)
void setAllLEDs(CRGB color) {
    if (!leds) return;
    fill_solid(leds, NUM_LEDS, color);
    // FastLED.show() burada çağrılmalı mı? Genellikle evet.
    FastLED.show();
}


// ==================== WEB İŞLEYİCİLERİ (HANDLERS) ====================

// --- Ayarları JSON olarak gönder ---
void handleGetConfig(AsyncWebServerRequest *request) {
    DynamicJsonDocument doc(2048); // Boyutu kontrol et
    doc["wifi_ssid"] = config.wifi_ssid;
    // Şifreyi gönderme!
    doc["eveningThresholdLux"] = config.eveningThresholdLux;
    doc["sleepThresholdLux"] = config.sleepThresholdLux;
...
// --- Mevcut sistem durumunu JSON olarak gönder ---
void handleGetStatus(AsyncWebServerRequest *request) {
    DynamicJsonDocument doc(1536); // Boyutu kontrol et
    doc["type"] = "status_update"; // WS ile aynı formatı kullanabiliriz
    ...
    doc["uptimeSeconds"] = state.uptimeSeconds; // state'den al
    doc["freeHeap"] = state.freeHeap; // state'den al
...
// --- Logları JSON olarak gönder ---
void handleGetLogs(AsyncWebServerRequest *request) {
    request->send(200, "application/json", getLogsJson());
}

// --- Radar Kalibrasyon İşlemleri ---
void handleCalibrate(AsyncWebServerRequest *request) {
    if (!request->hasParam("action", true)) { // POST parametresi mi?
    ...
    String responseMsg = "";
    uint16_t currentSmoothedAmplitude = sensorManager.getSmoothedRadarValue(); // Mevcut değeri al

    if (action == "setNoiseFloor") {
        uint16_t val = sensorManager.calibrateNoiseFloor(); // SensorManager'dan ölçümü al
        if (val != config.radarNoiseFloor) {
            config.radarNoiseFloor = val;
            // Diğer noktaların gürültüden yüksek olduğundan emin ol
            // DÜZELTME: max() fonksiyonu aynı tip argümanlar bekler. Cast yapalım.
        ...
        uint16_t val = sensorManager.captureCurrentAmplitude(); // Anlık (yumuşatılmış) değeri yakala
        if (val > config.radarNoiseFloor) {
         ...
         if (val > config.radarNoiseFloor) { // Gürültüden yüksek olmalı
             // İdealde val > config.radarAmplitudeAt1m olmalı, ama zorunlu kılmayalım
         ...
         uint16_t val = sensorManager.captureCurrentAmplitude();
          if (val > config.radarNoiseFloor) { // Gürültüden yüksek olmalı
              // İdealde val > config.radarAmplitudeAt3m olmalı
    ...
    // Kalibrasyon sonrası geçerliliği kontrol et
    state.calibrationInvalid = !(config.radarAmplitudeAt5m >= config.radarAmplitudeAt3m &&
                                 config.radarAmplitudeAt3m >= config.radarAmplitudeAt1m &&
                                 config.radarAmplitudeAt1m > config.radarNoiseFloor);
    ...
    if (calibrationChanged) {
        ...
        // Hata oluştu ama ayar değişmedi (örn. gürültüden düşük değer)
        addLog("Kalibrasyon Hatası: " + responseMsg);
        request->send(400, "text/plain", responseMsg);
    } else {
        // Değişiklik yok veya zaten aynı değerdi
        request->send(200, "text/plain", responseMsg + " - Değişiklik yok.");
    }
}

// --- 404 Bulunamadı Handler ---
void handleNotFound(AsyncWebServerRequest *request) {
    addLog("Bulunamadı (404): " + request->url());
    request->send(404, "text/plain", "404: Kaynak bulunamadi");
}

// --- HTTP Firmware Güncelleme Bitişi ---
void handleHttpUpdateFinished(AsyncWebServerRequest *request) {
     if (Update.hasError()) {
        // DÜZELTME: Update.errorString() zaten const char* döndürür, .c_str() gereksiz.
        addLog("HTTP Güncelleme Hatası: " + String(Update.errorString()));
        // Tarayıcıya daha detaylı hata gönder
        AsyncResponseStream *response = request->beginResponseStream("text/plain");
        response->printf("Update Failed Error[%u]: %s\n", Update.getError(), Update.errorString()); // .c_str() kaldırıldı
        ...
        delay(200); // Yanıtın gitmesi için kısa bekleme
        ESP.restart();
     }
}

// --- HTTP Firmware Dosyası Yükleme İşlemi ---
void handleHttpUpload(AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final) {
    if (index == 0) { // Yükleme başlangıcı
        ...
        // Gerekli boyut kontrolü (ESP32-C6'nın flash boyutuna göre)
        uint32_t maxSketchSpace = (ESP.getFreeSketchSpace() - 0x1000) & 0xFFFFF000;
         if (!Update.begin(maxSketchSpace)) { // U_FLASH varsayılan
             // DÜZELTME: String() ile çevirerek birleştirme
        ...
        // İsteğe bağlı: Güncelleme sırasında LED göstergesi
    ...
    if (final) { // Yükleme tamamlandı
        if (!Update.end(true)) { // true: başarıyla tamamla
            // DÜZELTME: String() ile çevirerek birleştirme
        ...
        // handleHttpUpdateFinished çağrılacak ve cihazı yeniden başlatacak.
    }
}

// --- Ayarları Yedekle (İndirme) ---
void handleBackupConfig(AsyncWebServerRequest *request) {
    if (!LittleFS.exists(CONFIG_FILE_PATH)) {
    ...
    // Dosya adını oluştur (örn: koridor-led-config-YYYYMMDD.json)
    // Zaman almak için NTP veya RTC gerekebilir, şimdilik sabit isim kullanalım
    String filename = String(HOSTNAME) + "-config_backup.json";

    // Tarayıcıya indirme olarak gönder
    AsyncWebServerResponse *response = request->beginResponse(
        LittleFS, CONFIG_FILE_PATH, "application/json", true // true: download
    );
    // response->addHeader("Content-Disposition", "attachment; filename=\"" + filename + "\""); // Gerekirse dosya adını ayarla
    request->send(response);
    addLog("Ayar dosyası yedeklemesi gönderildi.");
    // file.close(); // beginResponse dosyayı kendi kapatır
}

// --- Ayarları Geri Yükle (Yükleme) ---
void handleRestoreUpload(AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final) {
     String tempFilePath = "/restore_temp.json"; // Geçici dosya adı

     if (index == 0) { // Yükleme başlangıcı
        addLog("Ayar geri yükleme başladı: " + filename);
        if (!filename.endsWith(".json")) {
            addLog("HATA: Geri yükleme için sadece .json dosyaları kabul edilir.");
            // İsteği burada kesmek gerekebilir ama AsyncWebServer bunu doğrudan desteklemiyor olabilir.
            // final kısmında kontrol edip sileriz.
            return; // Hata mesajı gönderemiyoruz, final'de hallet
        }

        // Eski geçici dosyayı sil (varsa)
        if(LittleFS.exists(tempFilePath)) {
            LittleFS.remove(tempFilePath);
        }
        // Yeni geçici dosyayı aç
     ...
     if (len > 0 && restoreFile) {
        if (restoreFile.write(data, len) != len) {
             ...
             restoreFile.close();
             LittleFS.remove(tempFilePath); // Hatalı dosyayı sil
             // Hata yönetimi?
             return;
        }
     }

     if (final) { // Yükleme bitti
        addLog("Ayar dosyası yüklemesi tamamlandı: " + filename);
        if (restoreFile) {
            restoreFile.close(); // Dosyayı kapat
        }

        // Dosya adını tekrar kontrol et (ilk index'te hata olsa bile buraya gelebilir)
        if (!filename.endsWith(".json")) {
            addLog("HATA: Yüklenen dosya .json değil, geri yükleme iptal edildi.");
             LittleFS.remove(tempFilePath); // Geçici dosyayı sil
             // İstemciye hata gönderme şansı yok, log yeterli.
             // request->send(400, "text/plain", "Sadece .json kabul edilir"); // Burası çalışmaz
            return;
        }


        // Geçici dosyayı oku ve doğrula (JSON mu?)
        ...
        DynamicJsonDocument doc(2048); // Boyut?
        ...
        if (error) {
             ...
             LittleFS.remove(tempFilePath); // Geçersiz dosyayı sil
             // İstemciye hata?
             return;
        }

        // Geçerli JSON ise, eski config dosyasını silip yenisini kopyala
...
void handleResetConfig(AsyncWebServerRequest *request) {
    ...
    if (LittleFS.remove(CONFIG_FILE_PATH)) {
        ...
        delay(500); // Yanıtın gitmesi için bekle
        ESP.restart();
    } else {
        addLog("HATA: Ayar dosyası silinemedi (belki zaten yoktu).");
         request->send(500, "text/plain", "Ayar dosyası silinemedi.");
    }
    // Not: Varsayılan ayarlar bir sonraki açılışta loadConfiguration başarısız olunca
    // otomatik olarak saveConfiguration ile tekrar oluşturulacak.
}

// --- Yeni Gelen Ayarları İşle ve Kaydet ---
void handleSaveConfig(AsyncWebServerRequest *request, JsonVariant &json) {
    ...
    bool wifiChanged = false; // WiFi ayarları değişirse yeniden başlatma gerekebilir

    // DÜZELTME: Daha basit ve hataya daha az açık bir güncelleme yöntemi kullanalım.
    // Lambda yerine doğrudan her alanı kontrol edelim.
    ...
    if (jsonObj.containsKey("wifi_password")) {
        ...
        if (val.is<const char*>()) {
            ...
            // Boş şifre gelirse bir şey yapma (mevcutu koru)
        }
    }

    // --- Sayısal Alanlar ---
    // Helper macro for numeric updates
    ...
    UPDATE_NUMERIC_CONFIG(config.motionThresholdAmplitude, "motionThresholdAmplitude", uint16_t, 100, 1, 4095); // ADC range?
    ...
    #undef UPDATE_NUMERIC_CONFIG // Makroyu temizle
    ...
    if (configChanged) {
        ...
        if (saveConfiguration()) {
            ...
            if (wifiChanged) {
                 ...
                 addLog(responseMsg);
                 delay(1000); // Yanıtın gitmesi için bekle
                 ESP.restart();
            } else {
                 request->send(200, "text/plain", responseMsg);
                 addLog(responseMsg);
...
void onWsEvent(AsyncWebSocket *server, AsyncWebSocketClient *client, AwsEventType type, void *arg, uint8_t *data, size_t len) {
    switch (type) {
        case WS_EVT_CONNECT:
            ...
            // Bağlanan istemciye hemen mevcut durumu ve logları gönder
            sendWsUpdate(); // Tek istemciye gönderme fonksiyonu olsaydı daha iyi olurdu, şimdilik herkese gönderelim
            client->text(getLogsJson()); // Logları sadece bu istemciye gönder
            break;
        case WS_EVT_DISCONNECT:
            ...
            break;
        case WS_EVT_DATA:
            { // Scope for AwsFrameInfo
                AwsFrameInfo *info = (AwsFrameInfo*)arg;
                if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
                    data[len] = 0; // Null-terminate
                    String message = (char*)data;
                    DynamicJsonDocument doc(512); // Gelen komutlar için JSON
                    ...
                    if (!error && doc.containsKey("command")) {
                        ...
                        if (command == "getStatus") {
                            // Anlık durum zaten periyodik gönderiliyor, belki tek istemciye tekrar gönderilebilir
                             sendWsUpdate(); // Şimdilik herkese
                        } else if (command == "getLogs") {
                            client->text(getLogsJson()); // Sadece isteyen istemciye
                        } else if (command == "getConfig") {
                            // Ayarları JSON olarak hazırla (şifre hariç)
                            ...
                            client->text(output); // Sadece isteyen istemciye
                            ...
                            if(doc.containsKey("value")) {
                                ...
                                if (config.currentEffect != newEffect) {
                                    ...
                                    saveConfiguration(); // Efekt değişikliğini kaydet
                                }
                            }
                             ...
                             if(doc.containsKey("value")) {
                                ...
                                bool changed = false;
                                // Hangi parlaklığı güncelleyeceğimizi belirle (mevcut moda göre)
                                // Bu anlık kontrol olduğu için, belki de config'i değil,
                                // sadece state.currentBrightness'ı etkilemeli?
                                // Ama kullanıcı bunu kalıcı yapmak isteyebilir. Config'i güncelleyelim.
                                ...
                                if (changed) {
                                     ...
                                     saveConfiguration(); // Parlaklığı kaydet
                                }
                                // Anlık parlaklığı da hemen ayarlayalım (fade efekti yerine)
                                // FastLED.setBrightness(br); // Bu, updateLeds'deki fade'i bozabilir
                                // state.currentBrightness = br; // updateLeds zaten bunu yapacak
                             }
                        } else if (command == "setLedsOn") {
                             if(doc.containsKey("value")) {
                                bool lightsOnCmd = doc["value"].as<bool>();
                                if (state.lightsOn != lightsOnCmd) {
                                     // Bu komut otomasyonu bypass etmeli mi?
                                     // Şimdilik sadece state'i değiştirelim, updateLeds/updateSystemState
                                     // bir sonraki döngüde durumu düzeltebilir veya bu manuel durumu koruyabiliriz.
                                     // Manuel kontrol için ayrı bir state değişkeni eklemek daha iyi olabilir.
                                     // Şimdilik sadece state'i değiştirelim:
                                     state.lightsOn = lightsOnCmd;
                                     addLog(String("Isiklar manuel (WS) ") + (state.lightsOn ? "AÇILDI" : "KAPATILDI"));
                                     // Eğer manuel kontrol kalıcı olacaksa, motion timeout vs. devre dışı bırakılmalı.
                                     // Şimdilik otomasyonun üzerine yazmasına izin veriyoruz.
                                     // Eğer ışıklar manuel kapatıldıysa, belki lastMotionDetectedTime'ı sıfırlamak lazım?
                                     if (!state.lightsOn) {
                                        state.lastMotionDetectedTime = 0; // Timeout'u hemen bitir
                                     }
                                }
                             }
                        } else {
...
// Mevcut durumu tüm bağlı WebSocket istemcilerine gönderir
void sendWsUpdate() {
    if (ws.count() == 0) return; // Bağlı istemci yoksa gönderme

    DynamicJsonDocument doc(1536); // handleGetStatus ile aynı boyutta olabilir
    ...
    String output;
    serializeJson(doc, output);
    ws.textAll(output); // Tüm bağlı istemcilere gönder
}

// ==================== YARDIMCI FONKSİYONLAR ====================

// Değeri bir aralıktan diğerine haritalar (float)
float mapFloat(float x, float in_min, float in_max, float out_min, float out_max) {
    // Bölme sıfır hatasını önle
    if (abs(in_max - in_min) < 1e-6) { // Çok küçük bir sayı (epsilon)
...
// WiFi bağlantısını kontrol eder ve gerekirse yeniden bağlanmayı dener
void checkWifiConnection() {
     if (state.apMode) return; // AP modundaysa kontrol etme

     unsigned long currentMillis = millis();
     // Bağlantı koptuysa ve yeniden bağlanma zamanı geldiyse
     if (WiFi.status() != WL_CONNECTED && currentMillis - state.lastReconnectAttempt > WIFI_RECONNECT_INTERVAL_MS) {
        ...
        state.wifiConnected = false;
        // OTA aktifiz ve WiFi koparsa, OTA'yı durdurmak gerekebilir mi? Şimdilik hayır.
        state.lastReconnectAttempt = currentMillis;
        WiFi.disconnect(); // Önce bağlantıyı kes
        delay(100); // Kısa bekleme
        WiFi.mode(WIFI_STA); // STA modunu tekrar ayarla (gerekli mi?)
        WiFi.setHostname(HOSTNAME); // Hostname'i tekrar ayarla
        WiFi.begin(config.wifi_ssid, config.wifi_password); // Kayıtlı bilgilerle tekrar dene
        Serial.print("Yeniden baglaniliyor");
     } else if (WiFi.status() != WL_CONNECTED && state.wifiConnected) {
         // Henüz reconnect interval dolmadı ama bağlantı yok, durumu false yap
         // Bu durum, loop içinde WiFi.status() her kontrol edildiğinde log spam yapabilir.
         // Sadece interval dolduğunda log atmak daha iyi.
         // state.wifiConnected = false; // Bunu yapmayalım, sadece interval'de log atalım.
         if(Serial.availableForWrite()) { // Prevent blocking if Serial buffer is full
             Serial.print("."); // Bağlanma sırasında nokta yazdırmaya devam et (çok sık yazdırmamaya dikkat)
         }
     }
     else if (WiFi.status() == WL_CONNECTED && !state.wifiConnected) {
         // Bağlantı başarılı oldu (ilk bağlantı veya yeniden bağlantı)
         state.wifiConnected = true;
         IPAddress ip = WiFi.localIP();
         addLog("WiFi yeniden baglandi. IP: " + ip.toString());
         Serial.println("\nWiFi yeniden baglandi! IP: " + ip.toString());
         // OTA'yı (yeniden) başlat
         setupOTA();
     }
}
